...
Elm.Main.make = function (_elm) {
   ...
   var food = $Time.every(2 * $Time.second);
   var updatePoints = F3(function (food,
   food$,
   points) {
      return points + $List.length(food) - $List.length(food$);
   });
   var updatePlayer = F2(function (pos,
   player) {
   ...
   var initialPlayer = {_: {}
                       ...
                       ,radius: 15
                       ,velMag: 0};
   var initialEnemy = _U.replace([["pos"
                                  ,{ctor: "_Tuple2"
                                   ,_0: 500
                                   ,_1: 500}]
                                 ,["velMag",5]
                                 ...
                                 ,["color",$Color.darkRed]],
   initialPlayer);
   var initialFood = _U.replace([["radius"
                                 ,10]
   ...
   var vecLen = function (_v12) {
      return function () {
         switch (_v12.ctor)
         {case "_Tuple2":
            return $Basics.sqrt(_v12._0 * _v12._0 + _v12._1 * _v12._1);}
         _U.badCase($moduleName,
         "on line 65, column 17 to 36");
      }();
   };
   var vecOp = F3(function (op,
   _v16,
   _v17) {
      return function () {
         switch (_v17.ctor)
         {case "_Tuple2":
            return function () {
                 ...
                 {case "_Tuple2":
                    return {ctor: "_Tuple2"
                           ,_0: A2(op,_v16._0,_v17._0)
                           ,_1: A2(op,_v16._1,_v17._1)};}
                 _U.badCase($moduleName,
                 "on line 53, column 31 to 49");
              }();}
         _U.badCase($moduleName,
         "on line 53, column 31 to 49");
      }();
   });
   var vecAdd = vecOp(F2(function (x,
   y) {
      return x + y;
   }));
   var vecMul = vecOp(F2(function (x,
   y) {
      return x * y;
   }));
   var vecSub = vecOp(F2(function (x,
   y) {
      return x - y;
   }));
   var hit = F2(function (player,
   enemy) {
      return _U.cmp(vecLen(A2(vecSub,
      player.pos,
      enemy.pos)),
      player.radius + enemy.radius) < 0;
   });
   var updateFood = F2(function (player,
   food) {
      return A2($List.filter,
      function ($) {
         return $Basics.not(hit(player)($));
      },
      food);
   });
   var font = $Text.typeface(_L.fromArray(["impact"]));
   var startGameMessage = font($Text.fromString("Click to lock pointer and start game"));
   var playAgainMessage = font($Text.fromString("Click to play again"));
   var deadMessage = font($Text.fromString("You\'re dead"));
   var velMagIncrease = 1.0e-4;
   var updateEnemy = F3(function (dt,
   player,
   enemy) {
      return _U.replace([["pos"
                         ...
                         enemy.pos))]
                        ,["velMag"
                         ,enemy.velMag + dt * velMagIncrease]],
      enemy);
   });
   var updateGameInPlay = F3(function (dt,
   pos,
   game) {
      return function () {
         ...
         pos,
         game.player);
         var enemy$ = A3(updateEnemy,
         dt,
         player$,
         game.enemy);
         var food$ = A2(updateFood,
         player$,
         game.food);
         var points$ = A3(updatePoints,
         game.food,
         food$,
         game.points);
         return A2(hit,
         player$,
         enemy$) ? _U.replace([["state"
                               ,Dead]
                              ,["points",points$]],
      ...
      return function () {
         ...
         {case "Food":
            return function () {
                 ...
                 switch (_v29.ctor)
                 {case "Play":
                    return _U.cmp($List.length(g.food),
                      6) < 0 ? _U.replace([["food"
                                           ,A2($List._op["::"],
                                           e._0,
                                           g.food)]],
   ...
   var renderPoints = function (p) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: 0
                                    ,_1: hHeight - 30})($Graphics$Collage.scale(2)($Graphics$Collage.text(font($Text.fromString($Basics.toString(p))))));
   };
   var boundY = A2($Basics.clamp,
   0,
   height);
   var width = 800;
   var hWidth = width / 2;
   var toMove = function (_v33) {
      return function () {
         switch (_v33.ctor)
         {case "_Tuple2":
            return {ctor: "_Tuple2"
   ...
   var renderActor = function (_v37) {
      return function () {
         return $Graphics$Collage.move(toMove(_v37.pos))($Graphics$Collage.filled(_v37.color)($Graphics$Collage.circle(_v37.radius)));
      }();
   };
   var renderToCollage = function (game) {
      return function () {
         var _v39 = game.state;
         switch (_v39.ctor)
         {case "Dead":
            return _L.fromArray([$Graphics$Collage.scale(2)($Graphics$Collage.text(deadMessage))
                                ,$Graphics$Collage.move({ctor: "_Tuple2"
                                                        ,_0: 0
                                                        ,_1: -30})($Graphics$Collage.scale(1.5)($Graphics$Collage.text(playAgainMessage)))
                                ,renderPoints(game.points)]);
            case "Play":
            return A2($List._op["::"],
              renderPoints(game.points),
              $List.map(renderActor)(A2($List.append,
              game.food,
              _L.fromArray([game.player
                           ,game.enemy]))));
            case "Waiting":
   ...
   A2($Random.$float,0,height));
   var newFood = function (t) {
      return _U.replace([["pos"
                         ,$Basics.fst($Random.generate(randomPosition)($Random.initialSeed($Basics.round(t))))]],
      initialFood);
   };
   var render = F2(function (_v44,
   game) {
      return function () {
         switch (_v44.ctor)
         {case "_Tuple2":
   ...
   var requestPointerLock = Elm.Native.Port.make(_elm).outboundSignal("requestPointerLock",
   function (v) {
      return [];
   },
   $Signal.map($Basics.always({ctor: "_Tuple0"}))(A2($Signal.filter,
   $Basics.not,
   false)(A2($Signal.sampleOn,
   $Mouse.clicks,
   isLocked))));
   var exitPointerLock = Elm.Native.Port.make(_elm).outboundSignal("exitPointerLock",
   function (v) {
      return [];
   },
   $Signal.map($Basics.always({ctor: "_Tuple0"}))(A2($Signal.filter,
   ...
   $Keyboard.isDown(27),
   isLocked))));
   var movement = Elm.Native.Port.make(_elm).inboundSignal("movement",
   "(Float, Float)",
   function (v) {
   ...
   var position = A3($Signal.foldp,
   F2(function (p,p$) {
      return boundedVec(A2(vecAdd,
      p,
      p$));
   }),
   ...
   var events = $Signal.mergeMany(_L.fromArray([A2($Signal.map,
                                               ...
                                               $AnimationFrame.frame)(A2($Signal.sampleOn,
   ...
   _elm.Main.values = {_op: _op
                      ...
                      ,updateGameInPlay: updateGameInPlay
                      ,updateGame: updateGame
                      ,renderPoints: renderPoints
                      ,renderActor: renderActor
                      ,renderToCollage: renderToCollage
                      ,render: render
                      ,food: food
                      ,position: position
                      ,events: events
